...
public class AutoReconnect extends Module {
    ...
    public Listener<GuiScreenEvent.Displayed> displayedListener = new Listener<>(event -> {
        if (isEnabled() && event.getScreen() instanceof GuiDisconnected && (cServer != null || mc.currentServerData != null))
            event.setScreen(new HiroshimaGUIDisconnected((GuiDisconnected) event.getScreen()));
    });

    private class HiroshimaGUIDisconnected extends GuiDisconnected {

        int millis = seconds.getValue() * 1000;
        long cTime;

        public HiroshimaGUIDisconnected(GuiDisconnected disconnected) {
