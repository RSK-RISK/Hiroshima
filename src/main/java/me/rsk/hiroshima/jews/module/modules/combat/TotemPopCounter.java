...
import me.rsk.hiroshima.gasda.event.events.TotemPopEvent;
...
public class TotemPopCounter extends Module {
    ...
    private Setting<colour> mode = register(Settings.e("Colour", colour.WHITE));
        ...
        for(EntityPlayer player : mc.world.playerEntities) {
            if(player.getHealth() <= 0) {
                if(popList.containsKey(player.getName())) {
                    Command.sendChatMessage(colourchoice() + player.getName() + " is now dead after popping " + popList.get(player.getName()) + " totems!");
    ...
    public Listener<PacketEvent.Receive> totemPopListener = new Listener<>(event -> {
        ...
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35) {
                ...
                HiroshimaClient.EVENT_BUS.post(new TotemPopEvent(entity));
            }
        }
