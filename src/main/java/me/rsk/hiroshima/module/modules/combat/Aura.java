...
import me.rsk.hiroshima.module.modules.misc.AutoTool;
...
import me.rsk.hiroshima.util.LagCompensator;
...
public class Aura extends Module {
    ...
    private Setting<Boolean> swordOnly = (Settings.b("SwordOnly", false));
    private Setting<Boolean> cacheck = (Settings.b("CADisable", false));
    private Setting<Boolean> alert = register(Settings.b("AnnounceUsage", true));
    private Setting<Boolean> TpsSync = register(Settings.b("TpsSync", false));
    private Setting<Boolean> gap = (Settings.b("OffHandGap", false));
    ...
    public void onEnable() {
        if (Criticals.getValue()) {
            ModuleManager.getModuleByName("CriticalAttacks").enable();
        } else {
            return;
        }
        if (TpsSync.getValue()) {
        ...
        if (this.alert.getValue() && Aura.mc.world != null) {
            Command.sendRawChatMessage("[KillAura]" + (ChatFormatting.GREEN) + " Enabled!");
        }
    }
    @Override
    public void onDisable() {
        if (Criticals.getValue()) {
            ModuleManager.getModuleByName("CriticalAttacks").disable();
        } else {
            return;
        }
        if (TpsSync.getValue()) {
        ...
        if (this.alert.getValue() && Aura.mc.world != null) {
            Command.sendRawChatMessage("[KillAura]" + (ChatFormatting.RED) + " Disabled!");
        }
 ...
 {
     if(cacheck.getValue())
         if(((HiroshimaCA)ModuleManager.getModuleByName("HiroshimaCA")).isEnabled()) return;
