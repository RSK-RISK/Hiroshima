...
import me.rsk.hiroshima.event.events.EntityConnectEvent;
...
public class LogoutSpots extends Module {
    ...
    public Setting<Integer> aSetting = register(Settings.integerBuilder("Alpha").withMinimum(0).withMaximum(255).withValue(170).build());
    ...
    public Listener<EntityConnectEvent.Leave> leaveEvent = new Listener<>(event -> {
        ...
        if (player != null && mc.player != null && !mc.player.equals(player)) {
            ...
            if (debugSetting.getValue()) {
                Command.sendChatMessage(player.getName() + " EZ LOG " + player.getPositionVector());
            }
        }
    ...
    public void onWorldRender(RenderEvent event) {
        ...
        for(Map.Entry<String, Vec3d> entry : loggedPlayers.entrySet()) {
            ...
            String str = name + " EZ log at X: " + x + " Y: " + y + " Z: " + z;
            HiroshimaTessellator.prepare(GL_LINE_LOOP);
            HiroshimaTessellator.drawBoxWithVec3d(pos, rSetting.getValue(), gSetting.getValue(), bSetting.getValue(), aSetting.getValue(), GeometryMasks.Quad.ALL);
    ...
    private void fireEvents(SPacketPlayerListItem.Action action, PlayerInfo info, GameProfile profile) {
        ...
        switch (action) {
            case ADD_PLAYER: {
                HiroshimaClient.EVENT_BUS.post(new EntityConnectEvent.Join(info, profile));
                break;
            }
            case REMOVE_PLAYER: {
                HiroshimaClient.EVENT_BUS.post(new EntityConnectEvent.Leave(info, profile));
                break;
            }
        }
    }
